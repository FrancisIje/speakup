...
class _DefinitionScreenState extends State<DefinitionScreen> {
  ...
  //   getRandomWord(userLang);
  // }

  Future getRandomWord(String userLanguage) async {
    var wordRes = await ChatGPTApi(context).generateWord(userLanguage);
    var meaningRes = await ChatGPTApi(context).getChatCompletion(
        question:
            "What is the meaning of $wordRes in 10 to 20 words in $userLanguage, use basic words such that non $userLanguage speakers can easily understand",
        learnLang: "English");
    var exampleRes = await ChatGPTApi(context).getChatCompletion(
        learnLang: "English",
        question:
            "Give 2 example sentences involving $wordRes in 10 to 15 words in $userLanguage, use basic words such that non $userLanguage speakers can easily understand");
  ...
  Widget build(BuildContext context) {
    userLang = Provider.of<UserInfoProvider>(context).user?.targetLanguage ??
        "English";
    return FutureBuilder(
        future: getRandomWord(userLang),
